...
% Division into training/testing set (parameter size_training)
size_training = 0.5;
random_permutation = randperm(size(C,1));
training_set = random_permutation(1:round(size(C,1)*size_training));
testing_set = random_permutation(round(size(C,1)*size_training)+1:end);
training_names = names(training_set);
testing_names = names(testing_set);


% Prepare training features
TRN_cover = imnoise(C(training_set,:),'gaussian',0,0.00001);
