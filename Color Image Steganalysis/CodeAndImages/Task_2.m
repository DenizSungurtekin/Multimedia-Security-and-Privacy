...
reconstruct = I;
reconstruct(:,:,2) = uint8(G_mod);


figure,
  subplot(1,2,1), imshow(I), title('Original Image');
  subplot(1,2,2), imshow(reconstruct), title('Image with secret message');


imwrite(reconstruct,'TowerLSB.png')
...
% Compare both images
pixelwise = imabsdiff(G,uint8(G_mod));
distortion = sum(sum(pixelwise.^2))
PSNR = psnr(G,uint8(G_mod))
StructuralSimilarity = SSIM(G,uint8(G_mod))